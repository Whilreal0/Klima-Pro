...
const Header: React.FC<HeaderProps> = ({ currentRoute, onCloseChatbot, isMobileMenuOpen: externalIsOpen, setIsMobileMenuOpen: externalSetIsOpen }) => {
    ...
    const getLinkStyles = (href: string, isDropdown = false) => {
        ...
        return {
            wrapper: `group relative px-3 py-6 inline-flex flex-col items-center text-sm font-semibold tracking-wide leading-none transition-colors duration-300 ${
                isActive ? 'text-primary' : 'text-gray-600 hover:text-primary'
            }`,
            indicator: `mt-2 h-0.5 rounded-full self-center transition-all duration-300 ${
                isActive ? 'w-8 bg-primary' : 'w-0 bg-primary/60 group-hover:w-8'
            }`
        };
    };
    ...
    return (
        <>
            {/* Announcement Bar */}
            <div className={`bg-primary text-white text-center py-2 px-4 transition-all duration-300 ${
                announcementVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full pointer-events-none'
            }`}>
            ...
            <header className={`bg-white/90 backdrop-blur-lg sticky top-0 z-50 border-b border-gray-100 transition-all duration-300 ${
